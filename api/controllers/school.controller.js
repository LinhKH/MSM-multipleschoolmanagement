...
export const registerSchool = (req, res) => {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      hashPassword = bcrypt.hashSync(fields.password[0], salt);
      ...
      const newSchool = await school.save((err, result) => {
        if (err) {
          return res.status(400).json({
...
export const updateSchoolById = async (req, res) => {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      res.status(200).json({
        success: true,
        message: "Update school by id successfully!",
        school,
      });
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Update school by id failed!",
    });
  }
};
